...
class ControllerExtensionModuleLightshopBlog extends Controller {
	public function index($settings) {
      ...
      // << Related Options / Связанные опции  
		...
		foreach ($this->model_catalog_lightshopblog->getBlogs($filter_data) as $result) {
			$data['blogs'][] = array(
				'title' => $result['title'],
	...
	public function write() {
      ...
      // << Related Options / Связанные опции  
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				$this->load->model('catalog/lbcomment');

				$this->model_catalog_lbcomment->addComment($this->request->get['blog_id'], $this->request->post);
	...
	public function getblog() {
      ...
      // << Related Options / Связанные опции  
		...
		$pathInfo = $this->model_catalog_lightshopcatblog->getPathByBlog($blog_id);
		...
		if($pathInfo){
			...
			foreach ($parts as $path_id) {
				...
				$category_info = $this->model_catalog_lightshopcatblog->getBlogCategory($path_id);
				if ($category_info) {
					$data['breadcrumbs'][] = array(
		...
		$category_info = $this->model_catalog_lightshopcatblog->getBlogCategory($category_id);
		if ($category_info) {
			$data['breadcrumbs'][] = array(
				'text' => $category_info['name'],
		...
		$blog_info = $this->model_catalog_lightshopblog->getBlog($blog_id);
		...
		$data['comments'] = $this->model_catalog_lbcomment->getCommentsByBlogId($blog_id);
		foreach ($data['comments'] as $key => $comment) {
			$data['comments'][$key]['date_added'] = $this->rus_date("j F, Y ", strtotime($comment['date_added']));
		}	

		$data['totalComments'] = $this->model_catalog_lbcomment->getTotalCommentsByBlogId($blog_id);
		...
		if ($this->config->get('theme_lightshop_blog_pdata')) {
			...
			$data['tags'] = array();
			$tag_info = $this->model_catalog_lightshopblog->getBlogTag($blog_id);
			if($tag_info){
				foreach($tag_info as $tag){
					$data['tags'][] = array(
						'title'			=>  $tag['tag'],
						'href'			=>  $this->url->link('extension/module/lightshop_blog/getbloglist', 'lbtag=' . $tag['tag'])
					);
				}
			...
			$results = $this->model_catalog_lightshopblog->getBlogRelated($this->request->get['blog_id']);

			foreach ($results as $result_id) {
				$result = $this->model_catalog_lightshopblog->getBlog($result_id);

				if ($this->config->get('theme_lightshop_image_blog_related_resize')) {
					if ($result['image']) {
						$image = $this->model_tool_image->lightshop_resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_blog_related_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_related_height'));
					} else {
						$image = $this->model_tool_image->lightshop_resize('placeholder.png', $this->config->get($this->config->get('config_theme') . '_image_blog_related_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_related_height'));
					}
				} else {
					if ($result['image']) {
							$image = $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_blog_related_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_related_height'));
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', $this->config->get($this->config->get('config_theme') . '_image_blog_related_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_related_height'));
					}
				}
			...
			$results = $this->model_catalog_lightshopblog->getBlogRelatedProd($this->request->get['blog_id']);
			...
			if ($blog_info['image']) {
				$image = $this->config->get('theme_lightshop_image_blog_item_resize') ? $this->model_tool_image->lightshop_resize($blog_info['image'], $this->config->get($this->config->get('config_theme') . '_image_blog_item_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_item_height')) : $this->model_tool_image->resize($blog_info['image'], $this->config->get($this->config->get('config_theme') . '_image_blog_item_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_item_height'));
			...
			$this->model_catalog_lightshopblog->updateViewed($blog_id);
	...
	public function getbloglist() { 
      ...
      // << Related Options / Связанные опции  
		...
		if(!$this->model_catalog_lightshopblog->isModuleSet()){
		...
		foreach ($this->model_catalog_lightshopblog->getBlogs($filter_data) as $result) {
			...
			$blogCat = array();
			$catinfo = $this->model_catalog_lightshopblog->getBlogCat($result['blog_id']);

			$commentCount = $this->model_catalog_lbcomment->getTotalCommentsByBlogId($result['blog_id']);
			...
			if($result['image']){
				$image = $this->config->get('theme_lightshop_image_blog_cat_resize') ? $this->model_tool_image->lightshop_resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_blog_cat_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_cat_height')) : $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_blog_cat_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_cat_height'));
			} else {
				$image = $this->model_tool_image->resize('no_image.png', $this->config->get($this->config->get('config_theme') . '_image_blog_cat_width'), $this->config->get($this->config->get('config_theme') . '_image_blog_cat_height'));
			}
		...
		$totalBlogs = $this->model_catalog_lightshopblog->getTotalBlogs($filter_data);
