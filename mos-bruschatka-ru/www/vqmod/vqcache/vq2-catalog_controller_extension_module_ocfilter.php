...
class ControllerExtensionModuleOCFilter extends Controller {
  ...
  public function initialise() {
    ...
    // gulp$this->document->addScript('catalog/view/javascript/ocfilter/ocfilter.js');
    ...
    $index = 0;
      ...
      if ($option['type'] != 'slide' && $option['type'] != 'slide_dual') {
          ...
          if ($count || !$this->config->get('ocfilter_hide_empty_values')) {
              ...
              $image = $this->model_tool_image->getImage($value['image']);
						} else {
							$image = false;
						}
            ...
            $params = $this->getValueParams($option['option_id'], $value['value_id'], $option['type']);
              ...
              'image'    => $image,
	            'params'   => $params,
							'count'    => $count,
              'selected' => $this_value,
              'href'     => $this->url->link('product/category', 'path=' . $this->request->get['path']) . $option['keyword'] . '/' .$value['keyword']
	          );
					}
        }
