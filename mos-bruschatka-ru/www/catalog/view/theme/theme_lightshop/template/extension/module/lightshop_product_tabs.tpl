						...
						<li class="catalogue__products-list-item">
							<div class="products-list__link" >
										<a class="products-list__img" href="<?php echo $product['href']; ?>">
											...
											<?php if($product['isnewest']) { ?>
												<div class="label products-list__label"><?php echo $labelsinfo['new']['name'][$language_id]; ?></div>
											<?php } ?>
											<?php if($product['special'] && $product['sales']) { ?>
												<div class="label products-list__label label--red"><?php echo $labelsinfo['sale']['name'][$language_id] . ' ' . $product['discount']; ?></div>
											<?php } ?>
					...
					<div class="catalog-min__tabs-container">
					  ...
					  <?php foreach ($product_tabs as $tab => $product_tab) { ?>
						...
						<div class="tabs-content js-tab-content <?php if(!$y) { ?> active <?php } ?>">
						  ...
						  <?php for ($t = 0; $t < ceil(count($products[$tab]) / $tp_limit); $t++) { ?>						    
							  <div class="catalog-min__row">
								<?php $y_min = $t*$tp_limit; $y_max = ($t+1)*$tp_limit; ?>
