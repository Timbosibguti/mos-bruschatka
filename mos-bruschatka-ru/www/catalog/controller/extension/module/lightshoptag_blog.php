...
//		$limit = $this->config->get('theme_lightshop_blog_limit');
		...
		foreach ($this->model_catalog_lightshopblog->getBlogsTag($filter_data) as $result) {

			$data['tags'][] = array(
				'tag' => trim($result['tag']),
				'total' => $result['total'],
				'href'  => $this->url->link('extension/module/lightshop_blog/getbloglist', 'lbtag=' . trim($result['tag']))
			);
		}	
		
		$totalBlogs = $this->model_catalog_lightshopblog->getBlogsTotal();
