...
class ControllerExtensionModuleLightshopcatblog extends Controller {
	public function index() {
		...
		$blogcategories = $this->model_catalog_lightshopcatblog->getBlogCategories(0);

		foreach ($blogcategories as $blogcategory) {
			$children_data = array();

			if ($blogcategory['category_id'] == $data['category_id']) {
				$children = $this->model_catalog_lightshopcatblog->getBlogCategories($blogcategory['category_id']);
				...
				foreach($children as $child) {
					...
					$children_data[] = array(
						...
						'name' => $child['name'],
						'href' => $this->url->link('extension/module/lightshopcat_blog', 'lbpath=' . $blogcategory['category_id'] . '_' . $child['category_id'])
					);
				}
			}
			...
			$data['blogcategories'][] = array(
				...
				'count'        => $this->model_catalog_lightshopblog->getTotalBlogs($filter_data),
				'children'    => $children_data,
				'href'        => $this->url->link('extension/module/lightshopcat_blog/getcat', 'lbpath=' . $blogcategory['category_id'])
			);
		}

		return $this->load->view('extension/module/lightshopcat_blog', $data);
	}
...
#			'href' => $this->url->link('extension/module/lightshop_blog/getbloglist')
...
//lightshop start
			...
			if ($category_id) {
				...
				$blogCat = array();
				$catinfo = $this->model_catalog_lightshopcatblog->getBlogCategory($category_id);
				if($catinfo){
					$blogCat = array(
						'name'		=>	$catinfo['name'],
						'href'		=>	$this->url->link('extension/module/lightshop_blog/getcat', 'lbpath=' . $catinfo['category_id'])
					);
			...
			foreach ($parts as $path_id) {
				...
				if ($category_info) {
					$data['breadcrumbs'][] = array(
						...
						'href' => $this->url->link('extension/module/lightshop_blog/getcat', 'lbpath=' . $path . $url)
					);
				}		
			}
		}
		...
		foreach ($this->model_catalog_lightshopblog->getBlogs($filter_data) as $result) {
			...
			$hrefBlog = $this->url->link('extension/module/lightshop_blog/getblog', 'blog_id=' . $result['blog_id']);
			if($this->config->get($this->config->get('config_theme') . '_blog_path')){
				$hrefBlog = $this->url->link('extension/module/lightshop_blog/getblog','lbpath=' . $category_id . '&blog_id=' . $result['blog_id']);
			}
		...
		$totalBlogs = $this->model_catalog_lightshopcatblog->getBlogsTotalByCategoryId($category_id);
			$pagination = new Pagination();
			$pagination->total = $totalBlogs;
			$pagination->page = $page;
			$pagination->limit = $limit;
			$pagination->url = $this->url->link('extension/module/lightshopcat_blog/getcat', 'lbpath=' . $category_id . '&page={page}');
