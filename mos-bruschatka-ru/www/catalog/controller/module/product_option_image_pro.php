...
class ControllerModuleProductOptionImagePro extends Model {
	...
	public function get_products_list_images() {
		...
		if ( empty($this->request->post['products_ids']) ) {
			
			// probably redirect with losing POST data, return URI for recall
			$json['redirect'] = $this->request->server['REQUEST_URI'];
			
		} elseif ( is_array($this->request->post['products_ids'])) {
