		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if (is_file(DIR_IMAGE . $this->config->get('theme_lightshop_header_logo'))) {
					$data['logo'] = HTTP_SERVER . 'image/' . $this->config->get('theme_lightshop_header_logo');
				} else {
					$data['logo'] = '';
				}
				$data['header_text_logo'] = html_entity_decode($this->config->get('theme_lightshop_header_text_logo'), ENT_QUOTES, 'UTF-8');
				$data['lightshop_color'] = $this->config->get('theme_lightshop_color');
...
// lightshop
				...
				foreach ($data['social_navs'] as $key => $social_nav) {
					$data['social_navs'][$key]['icone'] = HTTP_SERVER . 'catalog/view/theme/theme_lightshop/img/icons/png/social_'.$social_nav['settype'].'.png';
				}
// lightshop end
...
// lightshop
					$query_product_info = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product['product_id'] . "'");	
