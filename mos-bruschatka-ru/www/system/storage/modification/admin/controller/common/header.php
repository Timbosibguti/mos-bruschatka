...
// lightshop			
			...
			$callbacks_new = $this->model_sale_callback->getСallbackNew();
			$callbacks_new = $callbacks_new['total'];

			$data['callbacksNewCount'] = $callbacks_new;
			$data['callbacks'] = $this->url->link('sale/callback', 'token=' . $this->session->data['token'] , true);

			$this->load->model('catalog/lightshopblog');
			$comments_new = $this->model_catalog_lightshopblog->getCommentNewCount(); 
			$comments_new = $comments_new['total'];
			$data['commentNewCount'] = $comments_new;
			$data['comments'] = $this->url->link('catalog/lightshopblog_review', 'token=' . $this->session->data['token'] , true);
// lightshop end

            

			
			$data['alerts'] = $customer_total + $comments_new + $product_total + $review_total + $return_total + $affiliate_total+$callbacks_new;//<-- Lightshop change this
