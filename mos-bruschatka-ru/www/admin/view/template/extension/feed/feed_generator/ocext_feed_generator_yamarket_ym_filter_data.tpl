        ...
        <tbody>
            ...
            <tr>
                    ...
                    <?php if($ym_categories && isset($ym_categories[ $category['category_id'] ]['category_id']) && $ym_categories[ $category['category_id'] ]['category_id']){ ?>
                        ...
                        <input checked="" type="checkbox"  name="ocext_feed_generator_yamarket_ym_filter_category[<?php echo $category['category_id'] ?>][category_id]" value="<?php echo $category['category_id'] ?>" />
                        <?php echo $category['name']; ?>
                        </div>
                    <?php }else{ ?>
                        <div style="min-height: 25px;">
                        <input type="checkbox"  name="ocext_feed_generator_yamarket_ym_filter_category[<?php echo $category['category_id'] ?>][category_id]" value="<?php echo $category['category_id'] ?>" />
                        <?php echo $category['name']; ?>
                        </div>
                    <?php } ?>
                    </td>
                    <td>
                    <?php if($templates_setting){ ?>
                        <select class="all_yml_export_ocext_ym_filter_data_categories_class form-control" name="ocext_feed_generator_yamarket_ym_filter_category[<?php echo $category['category_id'] ?>][setting_id]" >
                            <option value="0"><?php echo $text_ym_filter_data_templates_setting_0; ?></option>
                        <?php
        ...
        <tbody>
            ...
            <tr>
                    ...
                    <?php if($ym_manufacturers && isset($ym_manufacturers[ $manufacturer['manufacturer_id'] ]['manufacturer_id']) && $ym_manufacturers[ $manufacturer['manufacturer_id'] ]['manufacturer_id']){ ?>
                        <div style="min-height: 25px;">
                        <input checked="" type="checkbox"  name="ocext_feed_generator_yamarket_ym_filter_manufacturers[<?php echo $manufacturer['manufacturer_id'] ?>][manufacturer_id]" value="<?php echo $manufacturer['manufacturer_id'] ?>" />
                        <?php echo $manufacturer['name']; ?>
                        </div>
                    <?php }else{ ?>
                        <div style="min-height: 25px;">
                        <input type="checkbox"  name="ocext_feed_generator_yamarket_ym_filter_manufacturers[<?php echo $manufacturer['manufacturer_id'] ?>][manufacturer_id]" value="<?php echo $manufacturer['manufacturer_id'] ?>" />
                        <?php echo $manufacturer['name']; ?>
                        </div>
                    <?php } ?>
                    </td>
