...
class ControllerCatalogManufacturer extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		if (isset($this->request->post['logo']) && is_file(DIR_IMAGE . $this->request->post['logo'])) {
			$data['mini'] = $this->model_tool_image->resize($this->request->post['logo'], 20, 20);
		} elseif (!empty($manufacturer_info) && is_file(DIR_IMAGE . $manufacturer_info['logo'])) {
			$data['mini'] = $this->model_tool_image->resize($manufacturer_info['logo'], 20, 20);
		} else {
			$data['mini'] = $this->model_tool_image->resize('no_image.png', 20, 20);
		}
