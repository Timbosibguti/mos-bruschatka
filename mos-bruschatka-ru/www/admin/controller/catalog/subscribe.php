...
class ControllerCatalogSubscribe extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_extension_theme_lightshop->addSubscribe($this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_extension_theme_lightshop->editSubscribe($this->request->get['subscribe_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $subscribe_id) {
				$this->model_extension_theme_lightshop->deleteSubscribe($subscribe_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'] .  $url, true)
		);

		$data['heading_title'] = $this->language->get('heading_title');

		$data['module_install'] = $this->model_extension_theme_lightshop->tableExists();

		if ($data['module_install']) {
			$data['send'] = $this->url->link('catalog/subscribe/send', 'token=' . $this->session->data['token'] . $url, true);
			$data['email'] = $this->url->link('catalog/subscribe/email', 'token=' . $this->session->data['token'] . $url, true);
			$data['authorization'] = $this->url->link('catalog/subscribe/authorizationEmail', 'token=' . $this->session->data['token'] . $url, true);
			$data['insert'] = $this->url->link('catalog/subscribe/insert', 'token=' . $this->session->data['token'] . $url, true);
			$data['delete'] = $this->url->link('catalog/subscribe/delete', 'token=' . $this->session->data['token'] . $url, true);
			...
			$subscribe_total = $this->model_extension_theme_lightshop->getTotalSubscibe();

			$results = $this->model_extension_theme_lightshop->getSubscribers($dataF);
			...
			foreach ($results as $result) {
				...
				$action[] = array(
					...
					'href' => $this->url->link('catalog/subscribe/update', 'token=' . $this->session->data['token'] . '&subscribe_id=' . $result['subscribe_id'] . $url, true)
				);
			...
			$data['sort_email'] = $this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'] . '&sort=email' . $url, true);
			$data['sort_status'] = $this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
			...
			$pagination->url = $this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('catalog/subscribe_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['subscribe_id'])) {
			$data['action'] = $this->url->link('catalog/subscribe/insert', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/subscribe/update', 'token=' . $this->session->data['token'] . '&subscribe_id=' . $this->request->get['subscribe_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['subscribe_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$subscribe_info = $this->model_extension_theme_lightshop->getSubscribe($this->request->get['subscribe_id']);
		}
		...
		$this->response->setOutput($this->load->view('catalog/subscribe_form.tpl', $data));
	}
	...
	public function send() {
		...
		$subscribers = $this->model_extension_theme_lightshop->getSubscribers();

		if ($this->validateSendMail($subscribers)) {
			foreach ($subscribers as $subscriber) {
				if ($subscriber['status']) {
					$subscribe_descriptions = $this->model_extension_theme_lightshop->getEmailDescription();
	...
	public function authorizationEmail() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateAuthorizEmailForm()) {
			$this->model_extension_theme_lightshop->addAuthDescription($this->request->post['subscribe_authorization']);

			$this->session->data['success'] = $this->language->get('text_success_emailform');

			$this->response->redirect($this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'], true));
		}
		...
		if (isset($this->request->post['subscribe_authorization'])) {
			...
			$data['subscribe_authorization'] = $this->model_extension_theme_lightshop->getAuthDescription();
		}
		...
		$data['action'] = $this->url->link('catalog/subscribe/authorizationEmail', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/subscribe_mail_authorization.tpl', $data));
	}
    ...
    if ($this->config->get('config_editor_default')) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateEmailForm()) {
			$this->model_extension_theme_lightshop->addEmailDescription($this->request->post['subscribe_descriptions']);
		...
		if (isset($this->request->post['subscribe_descriptions'])) {
			...
			$data['subscribe_descriptions'] = $this->model_extension_theme_lightshop->getEmailDescription();
		}
		...
		$data['action'] = $this->url->link('catalog/subscribe/email', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('catalog/subscribe', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('catalog/subscribe_mail.tpl', $data));
	}
	...
	protected function validateForm() {
		...
		$check_email = $this->model_extension_theme_lightshop->checkEmail($this->request->post['email'], $subscribe_id);
