...
class ControllerExtensionModuleLightshopcatblog extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('lightshopcat_blog', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/lightshopcat_blog', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/module/lightshopcat_blog', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=module', true);

		if (isset($this->request->post['lightshopcat_blog_status'])) {
			$data['lightshopcat_blog_status'] = $this->request->post['lightshopcat_blog_status'];
		} else {
			$data['lightshopcat_blog_status'] = $this->config->get('lightshopcat_blog_status');
		}
